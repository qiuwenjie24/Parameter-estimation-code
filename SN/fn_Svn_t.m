% entanglement entropy AAH

function [vE_vn_all,A_fin_all,B_fin_all] = fn_Svn_t(N,th,lambda,beta,Time,Num_aver)
...
for jj1=1:tnum
    ...
    vP = fn_P_old(B, lambda, M);           %probability 1*N
%     yyy = fn_P(B, lambda, M);
%     if norm(vP-yyy)>1e-5
%         fprintf('fn_P_old is wrong \n')
%     end

    vE_vn(jj1) = fn_vN_Entropy_old(B,M);   %von Neumann entropy
%     xxx = fn_vN_Entropy(B,M);
%     if norm(vE_vn(jj1)-xxx)>1e-5
%         fprintf('fn_vN_Entropy_old is wrong \n')
%     end
