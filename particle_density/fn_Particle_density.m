...
function Aver_density = fn_Particle_density(beta, aver_num)
...
%% ------------------parfor--------------------------------
ALL_density = cell(1,aver_num);
parfor jj2=1:aver_num
    
    Particle_density = zeros(N, Tnum);
    ind_p = zeros(1, Tnum);
    vE_sh = zeros(1, Tnum);
    ...
    for jj1=1:Tnum
        ...
        Particle_density(:, jj1) = density;  %粒子数密度分布
        
    end
    
    ALL_density{jj2} = Particle_density;
end

%% 
Aver_density = zeros(N, Tnum);
for jj3=1:aver_num
    Aver_density = Aver_density + ALL_density{jj3}/aver_num;
end
