...
beta = 0.1;
Flie_num = 9;
FileName = ['New_Folder_Estimate_th0\estimate_W3'];
load(FileName,'th_sim','th')
col_num = size(th_sim,2);
th_sim_all = zeros(1,col_num*Flie_num);

for jj1 = 1:Flie_num
    FileName = ['New_Folder_Estimate_th',num2str(jj1-1),'\estimate_W3'];
...
MSE_th_W = th_sim_all_mse;
th_sim_all_W = th_sim_all;
th_sim_all_aver_W = th_sim_all_aver;
th_sim_all_std_W = th_sim_all_std;
Var_th_W = th_sim_all_std.^2;
leg_W = ['$\beta=$',num2str(beta)];

%% S
beta = 0.005;
Flie_num = 9;
FileName = ['New_Folder_Estimate_th0\estimate_S3'];
load(FileName,'th_sim','th')
col_num = size(th_sim,2);
th_sim_all = zeros(1,col_num*Flie_num);

for jj1 = 1:Flie_num
    FileName = ['New_Folder_Estimate_th',num2str(jj1-1),'\estimate_S3'];
    load(FileName,'th_sim')
    temp_col = [col_num*(jj1-1)+1 :1: col_num*jj1];
    th_sim_all(1,temp_col) = th_sim;
end
...
%                    'th_sim_all_aver_W','Var_th_W','MSE_th_W','leg_W')

%% Expected value E(th)
figure
plot(1:col_num*Flie_num,abs(th_sim_all_aver_W),'-','DisplayName',leg_W,'LineWidth',3)
hold on 
...
% xlswrite(['.\data_origin\Fig_Mean_X','.xlsx'],(1:col_num*Flie_num)');
% xlswrite(['.\data_origin\Fig_Mean_Yw','.xlsx'],abs(th_sim_all_aver_W)');
% xlswrite(['.\data_origin\Fig_Mean_Ys','.xlsx'],abs(th_sim_all_aver_S)');
% xlswrite(['.\data_origin\Fig_Real_Xr','.xlsx'],[1,col_num*Flie_num]');
% xlswrite(['.\data_origin\Fig_Real_Yr','.xlsx'],[2,2]');

%% Variance Var(th) 样本方差
figure
plot(1:col_num*Flie_num,Var_th_W,'-','DisplayName',leg_W,'LineWidth',3)
hold on 
...
% xlswrite(['.\data_origin\Fig_Var_X','.xlsx'],(1:col_num*Flie_num)');
% xlswrite(['.\data_origin\Fig_Var_Yw','.xlsx'],Var_th_W');
% xlswrite(['.\data_origin\Fig_Var_Ys','.xlsx'],Var_th_S');

%% MSE 均方误差  MSE(th)
figure
plot(1:col_num*Flie_num,MSE_th_W,'-','DisplayName',leg_W,'LineWidth',3)
hold on 
...
% xlswrite(['.\data_origin\Fig_MSE_X','.xlsx'],(1:col_num*Flie_num)');
% xlswrite(['.\data_origin\Fig_MSE_Yw','.xlsx'],MSE_th_W');
% xlswrite(['.\data_origin\Fig_MSE_Ys','.xlsx'],MSE_th_S');

%% Distribution of th
figure
edges = [-0.85:0.1:3.75];
hw = histogram(th_sim_all_W, edges, 'Normalization','probability');
